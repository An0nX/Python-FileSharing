...
def load_file():
    ...
    if file_size >= free_space:
        for node in nodes:
            try:
                free = requests.get(f"{node}/status", params={"key": password}).json()[
                    "free_space"
                ]
            except:
                continue
            if free > file_size:
                ...
                if response.status_code == 200:
                    ...
                    return (
                        jsonify({"file_link": f"{node}/{filename}"}),
                        200,
                        {"Content-Type": "application/json"},
                    )
                else:
        ...
        return (
            jsonify({"file_link": f"{server_url}/{filename}"}),
            200,
            {"Content-Type": "application/json"},
        )
